...
single_deep <- single_raw %>% filter(DP>20)

# filter
single <- mutation_filter(mut = single_deep, min_celltype = 10, min_mut_per_cell = 10, min_cell_one_mut = 15, meta = meta)
...
anno_color <- list(sample = color[c(1:10)], tissue = color[11:13])
...
matrix[matrix>=2] <- 2
graphics.off()
p1 <- pheatmap(matrix, annotation_col = meta[,c("tissue","sample")], cluster_cols = T, annotation_colors = anno_color,
               show_rownames = F, show_colnames = F,color = colorRampPalette(brewer.pal(9,"GnBu"))(50), clustering_distance_cols = "manhattan")
p2 <- pheatmap(matrix[,meta$cell[order(meta$sample)]], annotation_col = meta[,c("tissue","sample")], cluster_cols = F, annotation_colors = anno_color,
...
signature <- snv_to_sig(dat = single_keep[,c(1,2,3,4)], ref = "hg19")
pdf("signature_VAF.pdf", width = 8, height = 3)
show_signature(array = as.array(table(signature$signature)), name = "VAF")
dev.off()
...
gg.gap(plot=p1,segments=list(c(10000,20000)),ylim=c(0,5*10^5))
#add.legend(plot = p1,margin = c(top=1,right=1,bottom=200,left=300))
dev.off()
pdf("hist_VAF.pdf", width = 6, height = 5)
gg.gap(plot=p2,segments=list(c(30000,40000)),ylim=c(0,5*10^5))
