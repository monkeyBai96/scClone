...
for(i in 1:2){
  ...
  # filter
  single <- mutation_filter(mut = single_deep, min_celltype = 5, min_mut_per_cell = 5, min_cell_one_mut = 5, meta = meta)
  #single <- mutation_filter(mut = single_deep, min_celltype = 5, min_mut_per_cell = 2, min_cell_one_mut = 5, meta = meta)
...
for(i in 1:2){
  ...
  matrix[matrix>=2] <- 2
  graphics.off()
  p1 <- pheatmap(matrix, annotation_col = meta[,c("celltype","celltype_resolution")], cluster_cols = T, annotation_colors = anno_color,
                 show_rownames = F, show_colnames = F,color = colorRampPalette(brewer.pal(9,"GnBu"))(50), clustering_distance_cols = "manhattan")
  p2 <- pheatmap(matrix[,meta$cell[order(meta$celltype)]], annotation_col = meta[,c("celltype","celltype_resolution")], cluster_cols = F, annotation_colors = anno_color,
...
for(i in 1:2){
  ...
  # color
  anno_color <- list(celltype = celltypes_color)
  p <- pheatmap::pheatmap(distance[order,order], annotation_col = meta_order[,c("celltype","celltype_resolution")], annotation_colors = anno_color,
                          annotation_row = meta_order[,c("celltype","celltype_resolution")],
  ...
  p <- pheatmap(matrix_VAF[,meta$cell[order(meta$cluster_tem)]], annotation_col = meta[,c("cluster_tem","celltype","celltype_resolution")], cluster_cols = T, annotation_colors = anno_color,
                show_rownames = F, show_colnames = F,color = colorRampPalette(brewer.pal(9,"GnBu"))(50), clustering_distance_cols = "manhattan")
