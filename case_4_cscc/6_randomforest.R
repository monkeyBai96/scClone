...
for(i in 1:2){
  ...
  graphics.off()
  p1 <- pheatmap(matrix_rf, annotation_col = meta[,c("cluster_tem","celltype","celltype_resolution")], color = colorRampPalette(brewer.pal(9,"GnBu"))(50),annotation_colors = anno_color,
                 show_rownames = F, show_colnames = F, cluster_cols = T, clustering_distance_cols = "manhattan")
  p2 <- pheatmap(matrix_rf_order, annotation_col = meta_order[,c("cluster_tem","celltype","celltype_resolution")], color = colorRampPalette(brewer.pal(9,"GnBu"))(50),annotation_colors = anno_color,
