...
for(i in 1:2){
  ...
  meta_select <- meta %>% filter(paste0(meta$cluster_new, "_",meta$celltype) %in% table$Var1[which(table$Freq>9)]) %>% arrange(celltype, cluster_new)
  ...
  p3 <- ggplot(table, aes(area = Freq, fill = Var1, label = Var1, subgroup = Var2)) +
    ...
    geom_treemap_subgroup_border(color='white', size = 15)+
    scale_fill_manual(values = sankey_color)
  p3
...
for(i in 1:2){
  ...
  table(meta$cluster_new ,meta$celltype)
  table <- as.data.frame(table(paste0(meta$cluster_new, "_",meta$celltype)))
  meta_select <- meta %>% filter(paste0(meta$cluster_new, "_",meta$celltype) %in% table$Var1[which(table$Freq>9)]) %>% arrange(cluster_new, celltype)
  table(meta_select$cluster_new ,meta_select$celltype)
  ...
  df <- meta_select %>% make_long(sample, cluster_new, celltype) 
  df$node <- factor(df$node, levels = c(unique(meta_select$sample),
                                        unique(meta_select$cluster_new),
                                        unique(meta_select$celltype)))
  p2 <- ggplot(df, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
