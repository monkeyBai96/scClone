...
for(i in 1:1){
  ...
  # filter
  single <- mutation_filter(mut = single_deep, min_celltype = 10, min_mut_per_cell = 10, min_cell_one_mut = 8, meta = meta)
...
for(i in 1:1){
  ...
  p <- gg.gap(plot=p1,segments=list(c(1000,2000)),ylim=c(0,4*10^4))
  #add.legend(plot = p1,margin = c(top=1,right=1,bottom=200,left=300))
  ggsave("hist_VAF_raw.pdf",p, width = 6, height = 5)
  
  p <- gg.gap(plot=p2,segments=list(c(1000,2000)),ylim=c(0,4*10^4))
