...
for(i in 1:2){
  ...
  ############################# SVM & signature ##################################
  # read data
  meta <- read.csv(paste0(path,"/0_exp/meta.csv"), row.names = 1) %>% filter(sample==sample2[i])
  ...
  ####################################### bulk
  bulk1 <- read.table("H:/scclone/PMID37612267/paper/wes/P3-PT_paired.hg19_multianno.txt", header = T ,sep = "\t", quote = "")
  bulk2 <- read.table("H:/scclone/PMID37612267/paper/wes/P3-HM_paired.hg19_multianno.txt", header = T ,sep = "\t", quote = "")
  bulk <- rbind(bulk1, bulk2)
  bulk$var <- paste0(bulk$Chr, "_", bulk$Start,"_",bulk$Ref,">",bulk$Alt)
  length(which(bulk$var %in% single_raw$var))
  ...
  single_snp$dbsnp1 <- ifelse(as.numeric(single_snp$dbsnp)>0.4 & single_snp$dbsnp!=".","yes","no")
  ...
  single_svm$mark <- ifelse(single_svm$var %in% bulk$var, "True", single_svm$mark) # bulk
