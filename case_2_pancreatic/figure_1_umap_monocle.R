...
for(i in 1:1){
  ...
  p1 <- DimPlot(object = data, reduction = 'tsne',label = TRUE, group.by = "seurat_clusters", pt.size = 2,raster = F)
  p2 <- DimPlot(object = data, reduction = 'tsne',label = TRUE, group.by = "celltype", pt.size = 2,raster = F, cols = celltype_color)
