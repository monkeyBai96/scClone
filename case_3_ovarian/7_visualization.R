...
for(i in c(3,7)){
  ...
  table(meta$cluster_new ,meta$celltype)
  table <- as.data.frame(table(paste0(meta$cluster_new, "_",meta$celltype)))
  meta_select <- meta %>% filter(paste0(meta$cluster_new, "_",meta$celltype) %in% table$Var1[which(table$Freq>2)]) %>% arrange(cluster_new, celltype)
  table(meta_select$cluster_new ,meta_select$celltype)
  
  # color
  celltype_color <- celltypes_color
  cluster_new_color <- c("cluster_1"=pal_nejm("default")(8)[5], "cluster_2"=pal_nejm("default")(8)[7], "cluster_3"=pal_nejm("default")(8)[8])
  heatmap_color <- list(celltype = celltype_color, cluster_new = cluster_new_color)
  sankey_color <- c(celltype_color,cluster_new_color)
