...
for(i in 2:3){
  ...
  # read data
  meta <- read.csv(paste0(path, "/0_exp/meta.csv"), row.names = 1) %>% filter(sample==HCC_name[i])
  ...
  single_deep <- single_raw %>% filter(DP>30)
  
  # filter
  if(i %in% c(1:2)){
    single <- mutation_filter(mut = single_deep, min_celltype = 10, min_mut_per_cell = 30, min_cell_one_mut = 30, meta = meta)
  }else if(i==3){
    single <- mutation_filter(mut = single_deep, min_celltype = 10, min_mut_per_cell = 30, min_cell_one_mut = 20, meta = meta)
  }else{
    single <- mutation_filter(mut = single_deep, min_celltype = 10, min_mut_per_cell = 20, min_cell_one_mut = 20, meta = meta)
  }
...
for(i in 2:3){
  ...
  # color
  n = length(unique(meta$celltype))
  color <- c(pal_npg("nrc")(10))[c(1:n)]
  show_col(color)
  names(color) <- c(unique(meta$celltype))
