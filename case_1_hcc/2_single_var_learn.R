...
for(i in 1:4){
  ...
  ################################ run SVM #######################################
  # read meta
  meta <- read.csv(paste0(path, "/0_exp/meta.csv"), row.names = 1)
  # read data
  ...
  single_raw <- single_raw %>% filter(DP>50 & DP<500)
  ...
  single_snp$dbsnp1 <- ifelse(as.numeric(single_snp$dbsnp)>0.2 & single_snp$dbsnp!=".","yes","no")
  ...
  bulk <- read.table(paste0(rna_path,"/Final_",HCC_name[i],"_T_anno.hg19_multianno.txt"), header = T, sep = "\t", quote = "")
  bulk$var <- paste0(bulk$Chr,"_",bulk$Start,"_",bulk$Ref,">",bulk$Alt)
  which(bulk$var %in% single_raw$var)
  signature_B <- snv_to_sig(dat = bulk[,c(1,2,4,5)], ref = "hg19")
  p <- show_signature(array = as.array(table(signature_B$signature)), name = "bulk")
  ggsave("signature_B.pdf",p$plot, width = 8, height = 3)
  ...
  single_svm$mark[which(single_svm$var %in% bulk$var)] <- "True"
  ...
  save$mark2 <- ifelse(save$var %in% bulk$var, "bulk", NA)
  write.csv(save, "single_svm.csv", row.names = F)
  write.csv(signature_T, "signature.csv", row.names = F)
  write.csv(signature_A, "signature_raw.csv", row.names = F)
}
